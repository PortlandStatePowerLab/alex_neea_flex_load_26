...
WEATHER_DIR = os.path.join(fl_dir, "Weather")
WEATHER_FILE = os.path.join(WEATHER_DIR, "USA_OR_Portland.Intl.AP.726980_TMY3.epw")
METADATA_DIR = os.path.join(fl_dir, "Metadata")
...
REG_DIR = os.path.join(fl_dir, "Reg Sig")
...
MAX_RESPONSE_CHANGE_KW_PER_INTERVAL = 1.5
...
# import numpy as np
# # Period is 62.83 min
# # frequency is 0.0002653 Hz or 0.1 rad/s
# duration_min = Duration * 24 * 60
# frequency = f"{t_res}min"
# period = int(duration_min / t_res)
# sim_times = pd.date_range(start=Start, periods=period, freq=frequency)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        ...
        while pending:
            ...
            if initialized_count > 0 and not first_building_initialized:
                ...
                percent_init = int((initialized_count / initial_home_count) * 100)
            ...
            # Report every 5 seconds.
                ...
                and time.monotonic() - last_report_time >= 10
            ):

                percent_init = int(100 * initialized_count / initial_home_count)
